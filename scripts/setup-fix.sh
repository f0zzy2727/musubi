...
# scaffold_pillar TEMPLATE DESTREL LABEL — create a DRAFT north-star pillar from
# a template if the dest file is absent. Uses globals proj/TEMPLATES/APPLY.
scaffold_pillar(){
  _dest="$proj/$2"
  if [ -f "$_dest" ]; then info "exists (untouched): $2"; return; fi
  gap "no real $3 — Oya boots blind on this pillar"
  info "would scaffold $2 from template (DRAFT — the command fills it)"
  if ask; then
    mkdir -p "$(dirname "$_dest")"
    { echo "<!-- DRAFT scaffold — replace with real content. Generated by setup-fix.sh. -->"; cat "$TEMPLATES/$1"; } > "$_dest"
    did "scaffolded $2"
  fi
}
...
while IFS= read -r toml; do
  ...
  # 1) north-star: each pillar (product vision, architecture) needs a real doc
  # in SOME recognized form. Scaffold a pillar only when it is missing entirely
  # — don't false-scaffold over an existing dir/overview form (e.g. an app whose
  # architecture lives in docs/architecture/ or docs/OVERVIEW.md).
  vis_ok=0
  for d in docs/PRODUCT-VISION.md docs/VISION.md docs/PRD.md; do
    [ -f "$proj/$d" ] && ! is_managed "$proj/$d" && vis_ok=1
  done
  arch_ok=0
  for d in docs/ARCHITECTURE.md docs/OVERVIEW.md; do
    [ -f "$proj/$d" ] && ! is_managed "$proj/$d" && arch_ok=1
  done
  for dd in docs/architecture docs/adr; do
    [ -d "$proj/$dd" ] && [ -n "$(ls -A "$proj/$dd" 2>/dev/null)" ] && arch_ok=1
  done
  ...
  # 2b) agent reference docs: capability registry + ship definition-of-done.
  # These are project-owned (forked once, then edited), so scaffold create-if-absent
  # and never overwrite. They kill two recurring time-sinks: agents refusing tasks
  # they have a path for, and releases declared done with a store product missing.
  for pair in "CAPABILITY-REGISTRY.md:docs/agents/capability-registry.md:capability registry (what the agents can reach + how)" \
              "SHIP-DOD.md:docs/agents/ship-dod.md:ship definition-of-done (catches absent release deliverables)"; do
    tmpl="${pair%%:*}"; rest="${pair#*:}"; destrel="${rest%%:*}"; label="${rest#*:}"
    dest="$proj/$destrel"
    if [ -f "$dest" ]; then
      info "exists (untouched): $destrel"
    else
      gap "no $label"
      info "would scaffold $destrel from template (project-owned; edit to your setup)"
      if ask; then
        mkdir -p "$(dirname "$dest")"
        { echo "<!-- DRAFT scaffold — edit to your setup. Generated by setup-fix.sh. -->"; cat "$TEMPLATES/$tmpl"; } > "$dest"
        did "scaffolded $destrel"
      fi
    fi
  done
  ...
  # absolute [comms].file is supported; only relative resolves against project
  case "$comms_rel" in /*) comms="$comms_rel" ;; *) comms="$proj/$comms_rel" ;; esac
  if [ ! -e "$comms" ]; then
